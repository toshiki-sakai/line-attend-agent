      ...
      for (const event of events) {
        ...
        switch (event.type) {
          ...
          case 'message':
            ...
            break;
          case 'postback':
            await handlePostback(tenant, event, env, flowEngine);
            break;
        }
...
async function handlePostback(
  ...
  if (result.success && result.booking) {
    ...
    // Execute booking_confirmed scenario step if defined
    const bookedStep = tenant.scenario_config?.steps?.find((s) => s.trigger === 'booking_confirmed');
    if (bookedStep) {
      await flowEngine.executeStep(tenant, endUser as EndUser, bookedStep, env);
    }
  } else {
    await pushMessage(
      tenant,
